  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-3 gap-3 animate-fade-up">
      <div v-for="(item, i) in summary" :key="i"
        :class="['bg-white rounded-xl border border-[#E2E8F0] p-4 shadow-sm', `delay-${i}00`]">
        <p class="text-xs text-[#64748B] font-medium uppercase tracking-wide">{{ item.label }}</p>
        <p :class="['font-mono text-lg font-bold mt-1', item.color]">₹{{ formatAmount(item.value) }}</p>
      </div>
    </div>

    <!-- ── Filters ────────────────────────────────────────────────────── -->
    <div class="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-4 animate-fade-up delay-100">
      <div class="flex flex-wrap gap-3 items-end">
        ...
        <div class="flex-1 min-w-48">
          ...
          <div class="relative">
            <svg class="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-[#94A3B8]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
            </svg>
            <input v-model="localFilters.search" type="text" placeholder="Search by note..."
        ...
        <div class="flex gap-2">
          ...
          <BaseButton variant="ghost" size="md" @click="resetFilters">Reset</BaseButton>
        </div>
      </div>
    </div>

    <!-- ── Passbook Table ─────────────────────────────────────────────── -->
    <div class="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden animate-fade-up delay-200">

      <!-- Table Header -->
      <div class="grid grid-cols-12 gap-2 px-5 py-3 bg-[#F8FAFC] border-b border-[#E2E8F0]">
        <div class="col-span-2 text-xs font-bold text-[#64748B] uppercase tracking-wide">Date</div>
        <div class="col-span-1 text-xs font-bold text-[#64748B] uppercase tracking-wide">Type</div>
        <div class="col-span-3 text-xs font-bold text-[#64748B] uppercase tracking-wide">Note</div>
        <div class="col-span-2 text-xs font-bold text-[#64748B] uppercase tracking-wide text-right">Amount</div>
        <div class="col-span-2 text-xs font-bold text-[#64748B] uppercase tracking-wide text-right">Before</div>
        <div class="col-span-2 text-xs font-bold text-[#64748B] uppercase tracking-wide text-right">Balance</div>
      </div>

      <!-- Loading -->
      <div v-if="txStore.isLoading" class="flex items-center justify-center py-16">
        <div class="w-6 h-6 border-2 border-[#E2E8F0] border-t-[#1A365D] rounded-full animate-spin" />
      </div>
      ...
      <div v-else-if="!txStore.transactions?.length"
        ...
        <p class="text-sm font-semibold text-[#64748B]">No transactions found</p>
      ...
      <div v-else>
        <div v-for="(tx, i) in (txStore.transactions ?? [])":key="tx._id"
          ...
          <div class="col-span-2">
            <p class="text-xs font-mono text-[#1E293B]">{{ formatDateShort(tx.date) }}</p>
          </div>

          <!-- Type badge -->
          <div class="col-span-1">
            <BaseBadge :variant="tx.type === 'deposit' ? 'deposit' : 'withdrawal'" size="sm">
              {{ tx.type === 'deposit' ? '↑' : '↓' }}
            </BaseBadge>
          </div>
          ...
          <div class="col-span-2 text-right">
            <p :class="['font-mono text-sm font-bold', tx.type === 'deposit' ? 'text-[#059669]' : 'text-[#DC2626]']">
          ...
          <!-- Before Balance -->
          <div class="col-span-2 text-right">
            <p class="font-mono text-xs text-[#94A3B8]">₹{{ formatAmount(tx.beforeBalance) }}</p>
          </div>

          <!-- After Balance -->
          <div class="col-span-2 text-right">
            <p class="font-mono text-sm font-semibold text-[#1A365D]">₹{{ formatAmount(tx.afterBalance) }}</p>
          </div>
        </div>
      </div>
    </div>
